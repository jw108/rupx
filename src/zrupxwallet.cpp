...
void CzRUPXWallet::GenerateDeterministicZRUPX(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
    ...
    uint512 seedZerocoin = GetZerocoinSeed(nCountLastUsed +1);
    LogPrintf("%s : Generated new deterministic mint. count=%d\n\t\tpubcoin=%s\n\t\tseed=%s\n", 
        __func__, nCountLastUsed+1, coin.getPublicCoin().getValue().GetHex().c_str(), seedZerocoin.GetHex().c_str());
}
