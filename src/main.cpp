...
int64_t GetBlockValue(int nHeight)
{
    ...
    } else if (nHeight < 1600 && nHeight > 0) {
        nSubsidy = 0.1 * COIN;
    } else if (nHeight < 20000 && nHeight >= 1600) {
        nSubsidy = 5 * COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZRupxStake)
{
    ...
    float percentageMultipler = nHeight < 20000 ? 0.25 : 0.75;
    
    //if this block is staked via zRupx, award the staker 10% more of the block reward.
    if(isZRupxStake)
        percentageMultipler -= 0.1;
    
    return blockValue * percentageMultipler;
}
    ...
    if (!listMissingCheckpoints.empty()) {
        ...
        while (pindex) {
            uiInterface.ShowProgress(
                ...
                std::max(1, 
                    std::min(99, (int)((double)(pindex->nHeight - Params().Zerocoin_StartHeight()) / (double)(chainActive.Height() - Params().Zerocoin_StartHeight()) * 100))));
    ...
    CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    //     RecalculateRUPAYASupply(Params().Zerocoin_StartHeight());
    // }
